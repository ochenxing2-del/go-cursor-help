...
# 获取用户选择
if ($AUTO_MODE) {
    $userChoice = $AUTO_CHOICE
    Write-Host "$BLUE🤖 [自动模式]$NC 已启用，默认选择: $userChoice"
    if ($userChoice -eq "1") {
        ...
        Write-Host "$RED❌ [错误]$NC 自动模式配置无效，仅支持 1 或 2"
        exit 1
    }
} else {
    do {
...
$confirmation = if ($AUTO_MODE) { $AUTO_CONFIRM_CONTINUE } else { Read-Host "是否继续执行？(输入 y 或 yes 继续，其他任意键退出)" }
